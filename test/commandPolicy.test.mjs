...
test('rejects write commands when REDIS_MODE=read', () => {
  const script = `
    ...
    try {
      ...
      if (!String(error.message).includes('REDIS_MODE=read')) {
        console.error(error);
        process.exit(2);
      }
    }
  ...
  const result = spawnSync(process.execPath, ['--input-type=module', '--eval', script], {
    cwd: process.cwd(),
    env: { ...process.env, REDIS_MODE: 'read' },
    encoding: 'utf8',
  });
...
test('supports REDIS_MODEL as a backward-compatible alias', () => {
  const script = `
    import { assertCommandAllowed } from './build/commandPolicy.js';
    try {
      assertCommandAllowed('SET', ['key', 'value']);
